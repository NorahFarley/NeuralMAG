...
__version__ = 'UnetHd_2D_2024.10.17'
...
class mmModel():
    ...
    def __init__(self, types="3DPBC", cell=(1,1,1),
        ...
        # 2D only!!!
        if self.size[2] != 1:
            print("  [ERROR] This is a special version for 2D cases")
            print("          Please make size[2] = 1\n")
            sys.exit(0)
        ...
        self.fftsize = 2 * np.array(self.size[:-1], dtype=int)
        self.pbc = np.array([0,0,0])
        ...
        self.FDMW = torch.zeros( (3,3) + ( self.fftsize[0], self.fftsize[1]//2 + 1), 
                                 dtype=torch.complex64, device=self.device)  # 2D
    ...
    def DemagInit(self):
        ...
        for pqw in pqw_range:
            pqw = np.array(pqw)
            R = 0.5*D + pqw*rvm
            RR = np.sqrt( np.einsum( 'ijl,ijl -> ij', R, R ) )
    ...
    def DemagField_FFT(self):
        ...
        M_tmp[:, :RN[0], :RN[1]] = (self.Spin.permute(3,0,1,2) * self.Msmx).reshape(3,RN[0],RN[1])
        FM = torch.fft.rfftn( M_tmp, dim=(1,2) ) #2D
        ...
        H = torch.fft.irfftn( FH, dim=(1,2) ).reshape(3,FN[0],FN[1],1)
