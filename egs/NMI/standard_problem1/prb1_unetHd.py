...
# Model shape
model = np.zeros(size, dtype=int)
model[28:228, 78:178, :] = 1
...
# Create MM model
film0 = MAG2305.mmModel(types="bulk", model=model, cell=cell, 
...
def plot_data(path, mh_name, spin_name, plt_name):
    ...
    ax2.quiver( spin[::4,::4,0].T, spin[::4,::4,1].T,
                spin[::4,::4,2].T, clim=[-0.5, 0.5] )
...
plot_data(path=path0, mh_name='MHx_data-unetHd', spin_name='Mrx_spin-unetHd', plt_name='MH_Mr-unetHd')
...
ax.plot(    data_mag[0], data_mag[1], label="LLG/Unet",
